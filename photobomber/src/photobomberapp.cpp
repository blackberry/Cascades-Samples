    ...
    mCameraHandle(CAMERA_HANDLE_INVALID)
{
  ...
  mSwitchCameraButton = Button::create("Switch Camera");

  mTakePictureButton = Button::create("Take Picture");
  mTakePictureButton->setVisible(false);

  QObject::connect(mSwitchCameraButton, SIGNAL(clicked()), this,
      SLOT(onSwitchCamera()));

  QObject::connect(mTakePictureButton, SIGNAL(clicked()), this,
      SLOT(onTakePicture()));
  ...
  Container* container =
      Container::create().layout(DockLayout::create()).add(
          Container::create().layoutProperties(
              DockLayoutProperties::create().horizontal(
                  HorizontalAlignment::Center).vertical(
                  VerticalAlignment::Center)).add(mViewfinderWindow)).add(
          Container::create().layoutProperties(
              DockLayoutProperties::create().horizontal(
                  HorizontalAlignment::Center).vertical(
                  VerticalAlignment::Bottom)).layout(
              StackLayout::create().direction(LayoutDirection::LeftToRight)).add(
              mSwitchCameraButton).add(mTakePictureButton));
...
void PhotoBomberApp::onWindowAttached(unsigned long handle,
...
int PhotoBomberApp::createViewfinder(camera_unit_t cameraUnit,
    const QString &group, const QString &id)
{
...
void PhotoBomberApp::shutterCallback(camera_handle_t handle, void *arg)
{
...
void PhotoBomberApp::stillCallback(camera_handle_t handle, camera_buffer_t *buf,
    void *arg)
{
  qDebug() << "stillCallback";
  PhotoBomberApp* inst = (PhotoBomberApp*) arg;
  if (buf->frametype == CAMERA_FRAMETYPE_JPEG)
  {
    ...
    if (camera_roll_open_photo(handle, &fd, filename, sizeof(filename),
...
void PhotoBomberApp::startFront()
{
  qDebug() << "onStartFront";
...
void PhotoBomberApp::stopCamera()
{
  qDebug() << "onStopCamera";
  if (mCameraHandle != CAMERA_HANDLE_INVALID)
  {
    ...
    mViewfinderWindow = ForeignWindow::create().id(QString("viewfinderWindow"));
    mViewfinderWindow->setWindowId(QString("cameraViewfinder"));
    Container *container = (Container*) oldViewfinderWindow->parent();
    container->replace(container->indexOf(oldViewfinderWindow),
        mViewfinderWindow);
...
void PhotoBomberApp::onTakePicture()
{
  qDebug() << "onTakePicture";
  if (mCameraHandle != CAMERA_HANDLE_INVALID)
  {
    camera_take_photo(mCameraHandle, &shutterCallback, NULL, NULL,
        &stillCallback, (void*) this, false);
    // disable take-picture button until we're ready to take a picture again
    mTakePictureButton->setEnabled(false);
  }
}

void PhotoBomberApp::onSwitchCamera()
{
  qDebug() << "onSwitchCamera";
  if (mCameraHandle != CAMERA_HANDLE_INVALID)
  {
    stopCamera();
