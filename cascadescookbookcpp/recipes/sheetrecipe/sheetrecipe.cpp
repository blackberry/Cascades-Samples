...
{
    // The new greetings text is stored so that the main text can be updated
...
Container *SheetRecipe::setUpRecipeContent()
{
    ...
    mGreetingsLabel = Label::create("Tap fruit for Sheet.").layoutProperties(
    ...
    mGreetingsLabel->textStyle()->setFontWeight(FontWeight::Light);

    Container *basketContainer = Container::create().layout(DockLayout::create()).topMargin(100);

    // The fruit is put inside a Container in order to off-set it with bottom padding, so
    // that it is aligned correctly (otherwise it would be hidden by the basket image below).
    Container *fruitContainer =
            Container::create().layout(StackLayout::create().bottom(160)).layoutProperties(
    ...
    ImageView *basketImage = ImageView::create("asset:///images/sheet/basket.png").layoutProperties(
        DockLayoutProperties::create().horizontal(HorizontalAlignment::Center).vertical(
    ...
    TapHandler *tapHandler = TapHandler::create().onTap(this,
...
void SheetRecipe::setUpModifySheet()
{
    ...
    TitleBar *modifyBar = TitleBar::create().title("Modify")
                  .acceptAction(saveAction).dismissAction(cancelAction);

    page->setTitleBar(modifyBar);

    // The buttons for cancel and add/update actions.
    Container *modifyContent = Container::create().layout(
        StackLayout::create().top(121 + 25).left(25).right(25));
    ...
    // a background image with a nice drop shadow, an overlaid text and the image of course.
    Container *imageSelectionContainer =
            Container::create().layout(DockLayout::create()).layoutProperties(
                StackLayoutProperties::create().horizontal(HorizontalAlignment::Center));
    ImageView *backPlate =
            ImageView::create("asset:///images/title_gui_buffet_empty_box").preferredWidth(170).layoutProperties(
                DockLayoutProperties::create().horizontal(HorizontalAlignment::Fill).vertical(
                    VerticalAlignment::Fill));

    Container *imageContainer = Container::create().layout(
        StackLayout::create().left(4).right(4).bottom(6));
    mModifyFruitImage = ImageView::create("asset:///images/sheet/fruit5.png").layoutProperties(
        StackLayoutProperties::create().horizontal(HorizontalAlignment::Center));

    Container *instructionContainer = Container::create().layout(
        StackLayout::create().left(25).right(25).top(10).bottom(10)).layoutProperties(
        StackLayoutProperties::create().vertical(VerticalAlignment::Fill).horizontal(
            HorizontalAlignment::Fill)).background(Color::fromARGB(0xaa272727));

    Label *instruction = Label::create("Tap to change").bottomMargin(50).layoutProperties(
        StackLayoutProperties::create().horizontal(HorizontalAlignment::Center));
    ...
    // Add the three different components to the Sheet content: Input field and image control.
...
void SheetRecipe::setUpFruitSheet()
{
    ...
    Container *newFruitContainer = Container::create().layout(
        StackLayout::create().top(121 + 25).left(25).right(25));
    ...
    listView->setListItemManager(itemfactory);

    connect(listView, SIGNAL(triggered(const QVariantList)), this, SLOT(onNewFruitChanged(const QVariantList)));
